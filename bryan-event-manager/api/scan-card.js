...
// Returns: { ok: true, fields: { first_name, last_name, email, company, role, phone } }
...
- Return ONLY the JSON, no markdown, no explanation, no code fences`
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({ ok: false, error: `OpenAI API error: ${errData.error?.message || response.statusText}` });
    }
