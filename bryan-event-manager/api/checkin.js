...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { rows } = await query(
      `SELECT id, first_name, last_name, email, company, role, checked_in, checked_in_at
       FROM participants
       WHERE LOWER(email) = $1
       LIMIT 1`,
      [email]
    ...
    if (action === 'check-in') {
      ...
      const upd = await query(
        `UPDATE participants
         ...
         WHERE id = $1
         RETURNING id, first_name, last_name, email, company, role, checked_in, checked_in_at`,
        [participant.id]
      );
