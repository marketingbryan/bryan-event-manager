...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!email) return res.status(400).json({ ok: false, error: 'Email missing' });

    const { rowCount, rows } = await query(
      `UPDATE participants SET rsvp = $1 WHERE LOWER(email) = $2
       RETURNING id, first_name, last_name, email, company, role, phone, rsvp, checked_in, checked_in_at`,
      [rsvp, email]
    );

    if (rowCount === 0) return res.status(404).json({ ok: false, error: 'Participant not found' });
