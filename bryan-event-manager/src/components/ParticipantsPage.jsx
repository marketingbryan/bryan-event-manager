...
export default function ParticipantsPage({ participants, loading, onCheckin, onUpload, onReset, onUpdateRsvp, hasData, authFetch }) {
  ...
  const [manualForm, setManualForm] = useState({ first_name: '', last_name: '', email: '', company: '', role: '', phone: '', rsvp: 'Invited' });
        ...
        const rows = results.data
          .map((r) => {
            ...
            return {
              ...
              phone: r.phone || r.telefono || r.tel || r.mobile || r.cell || r.cellulare || '',
              rsvp: rowRsvp || csvRsvp,
            };
          })
          .filter((r) => r.email);
        if (rows.length === 0) {
          setUploadError('No valid rows found. Expected columns: first_name, last_name, email (+ optional: company, role, phone, rsvp).');
          return;
        }
        onUpload(rows);
  ...
  const handleManualAdd = async (e) => {
    ...
    try {
      ...
      setManualForm({ first_name: '', last_name: '', email: '', company: '', role: '', phone: '', rsvp: 'Invited' });
  ...
  const handleAction = async (email, action) => {
    setBusy(email);
    try { await onCheckin(email, action); } finally { setBusy(null); }
  };
      ...
      <div className="bg-white rounded-xl border p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <button onClick={() => { setShowManualForm((v) => !v); setManualError(null); }} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50">
              {showManualForm ? 'Close' : '+ Add Manually'}
            </button>
            <button onClick={() => setShowCardScanner(true)} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50" title="Scan a business card">
              Scan Card
            </button>
            {hasData && (
              <button onClick={onReset} className="px-4 py-2 bg-white border border-red-300 text-red-600 text-sm font-medium rounded-lg hover:bg-red-50">
                Clear All
              </button>
            )}
          </div>
          ...
          <form onSubmit={handleManualAdd} className="border-t pt-4 mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <div>
                ...
                <input type="text" value={manualForm.first_name} onChange={(e) => setManualForm((f) => ({ ...f, first_name: e.target.value }))} placeholder="Mario" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Last Name</label>
                <input type="text" value={manualForm.last_name} onChange={(e) => setManualForm((f) => ({ ...f, last_name: e.target.value }))} placeholder="Rossi" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Email</label>
                <input type="email" value={manualForm.email} onChange={(e) => setManualForm((f) => ({ ...f, email: e.target.value }))} placeholder="mario.rossi@example.com" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Phone</label>
                <input type="tel" value={manualForm.phone} onChange={(e) => setManualForm((f) => ({ ...f, phone: e.target.value }))} placeholder="+39 333 1234567" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Company</label>
                <input type="text" value={manualForm.company} onChange={(e) => setManualForm((f) => ({ ...f, company: e.target.value }))} placeholder="Acme Inc." className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Role</label>
                <input type="text" value={manualForm.role} onChange={(e) => setManualForm((f) => ({ ...f, role: e.target.value }))} placeholder="Marketing Manager" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">RSVP</label>
                <select value={manualForm.rsvp} onChange={(e) => setManualForm((f) => ({ ...f, rsvp: e.target.value }))} className="w-full px-3 py-2 border rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand text-sm">
            ...
            <div className="mt-3 flex items-center gap-3">
              <button type="submit" disabled={manualSaving} className="px-4 py-2 bg-brand text-white text-sm font-medium rounded-lg hover:bg-brand-hover disabled:opacity-50">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-left">
              <tr>
                ...
                <th className="px-4 py-3 font-medium hidden xl:table-cell">Phone</th>
              ...
              {loading ? (
                ...
                filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600 hidden xl:table-cell">{p.phone || '—'}</td>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <button
                        ...
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium cursor-pointer transition-colors ${
                          p.rsvp === 'Registered' ? 'bg-blue-100 text-blue-800 hover:bg-blue-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        }`}
                      >
                        {p.rsvp || 'Invited'}
                      </button>
